...
func (w *WEX) UpdateTicker(p pair.CurrencyPair, assetType assets.AssetType) (ticker.Price, error) {
	...
	pairsCollated, err := exchange.FormatExchangeCurrencies(w.Name, w.GetEnabledPairs(assetType), assetType)
	if err != nil {
	...
	for _, x := range w.GetEnabledPairs(assetType) {
		currency := exchange.FormatExchangeCurrency(w.Name, x, assetType).Lower().String()
		var tp ticker.Price
		tp.Pair = x
...
func (w *WEX) UpdateOrderbook(p pair.CurrencyPair, assetType assets.AssetType) (orderbook.Base, error) {
	...
	orderbookNew, err := w.GetDepth(exchange.FormatExchangeCurrency(w.Name, p, assetType).String())
	if err != nil {
