...
func (h *HUOBIHADAX) UpdateTicker(p pair.CurrencyPair, assetType assets.AssetType) (ticker.Price, error) {
	...
	tick, err := h.GetMarketDetailMerged(exchange.FormatExchangeCurrency(h.Name, p, assetType).String())
	if err != nil {
...
func (h *HUOBIHADAX) UpdateOrderbook(p pair.CurrencyPair, assetType assets.AssetType) (orderbook.Base, error) {
	...
	orderbookNew, err := h.GetDepth(exchange.FormatExchangeCurrency(h.Name, p, assetType).String(), "step1")
	if err != nil {
