...
func (c *CoinbasePro) UpdateTicker(p pair.CurrencyPair, assetType assets.AssetType) (ticker.Price, error) {
	...
	tick, err := c.GetTicker(exchange.FormatExchangeCurrency(c.Name, p, assetType).String())
	if err != nil {
		return ticker.Price{}, err
	}

	stats, err := c.GetStats(exchange.FormatExchangeCurrency(c.Name, p, assetType).String())
...
func (c *CoinbasePro) UpdateOrderbook(p pair.CurrencyPair, assetType assets.AssetType) (orderbook.Base, error) {
	...
	orderbookNew, err := c.GetOrderbook(exchange.FormatExchangeCurrency(c.Name, p, assetType).String(), 2)
	if err != nil {
