...
func (b *Binance) UpdateOrderbook(p pair.CurrencyPair, assetType assets.AssetType) (orderbook.Base, error) {
	...
	orderbookNew, err := b.GetOrderBook(OrderBookDataRequestParams{Symbol: exchange.FormatExchangeCurrency(b.Name,
		p, assetType).String(), Limit: 1000})
	if err != nil {
...
func (b *Binance) CancelOrder(order exchange.OrderCancellation) error {
	...
	_, err = b.CancelExistingOrder(exchange.FormatExchangeCurrency(b.Name,
		order.CurrencyPair, order.AssetType).String(), orderIDInt, order.AccountID)
