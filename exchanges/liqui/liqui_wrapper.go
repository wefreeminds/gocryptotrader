...
func (l *Liqui) UpdateTicker(p pair.CurrencyPair, assetType assets.AssetType) (ticker.Price, error) {
	...
	pairsString, err := exchange.FormatExchangeCurrencies(l.Name,
		l.GetEnabledPairs(assetType), assetType)
	if err != nil {
	...
	for _, x := range l.GetEnabledPairs(assetType) {
		currency := exchange.FormatExchangeCurrency(l.Name, x, assetType).String()
		var tp ticker.Price
		tp.Pair = x
...
func (l *Liqui) UpdateOrderbook(p pair.CurrencyPair, assetType assets.AssetType) (orderbook.Base, error) {
	...
	orderbookNew, err := l.GetDepth(exchange.FormatExchangeCurrency(l.Name, p, assetType).String())
	if err != nil {
